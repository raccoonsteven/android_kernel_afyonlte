/*
 * drivers/cpufreq/cpufreq_zerolag.c
 *
...
static DEFINE_PER_CPU(struct cpufreq_zerolag_cpuinfo, cpuinfo);
...
static int cpufreq_governor_zerolag(struct cpufreq_policy *policy,
		unsigned int event);

#ifndef CONFIG_CPU_FREQ_DEFAULT_GOV_ZEROLAG
static
#endif
struct cpufreq_governor cpufreq_gov_zerolag = {
	.name = "zerolag",
	.governor = cpufreq_governor_zerolag,
	.max_transition_latency = 10000000,
	.owner = THIS_MODULE,
};

static void cpufreq_zerolag_timer_resched(
	struct cpufreq_zerolag_cpuinfo *pcpu)
{
 ...
 * function.
 */
static void cpufreq_zerolag_timer_start(int cpu, int time_override)
{
	struct cpufreq_zerolag_cpuinfo *pcpu = &per_cpu(cpuinfo, cpu);
	unsigned long flags;
	unsigned long expires;
	if (time_override)
...
static unsigned int choose_freq_grid(
   struct cpufreq_zerolag_cpuinfo *pcpu, unsigned int loadadjfreq, int cpu_load)
{
...
static unsigned int choose_freq(
	struct cpufreq_zerolag_cpuinfo *pcpu, unsigned int loadadjfreq)
{
...
static void cpufreq_zerolag_timer(unsigned long data)
{
	u64 now;
	unsigned int delta_time;
	u64 cputime_speedadj;
	...
	struct cpufreq_zerolag_cpuinfo *picpu;
	...
	if (pcpu->target_freq == new_freq) {
		trace_cpufreq_zerolag_already(
			data, cpu_load, pcpu->target_freq,
			pcpu->policy->cur, new_freq);
		goto rearm_if_notmax;
	}

	trace_cpufreq_zerolag_target(data, cpu_load, pcpu->target_freq,
					 pcpu->policy->cur, new_freq);
...
rearm:
	if (!timer_pending(&pcpu->cpu_timer))
		cpufreq_zerolag_timer_resched(pcpu);
...
static void cpufreq_zerolag_idle_start(void)
{
	struct cpufreq_zerolag_cpuinfo *pcpu =
...
static void cpufreq_zerolag_idle_end(void)
{
	struct cpufreq_zerolag_cpuinfo *pcpu =
		&per_cpu(cpuinfo, smp_processor_id());
	...
	if (!timer_pending(&pcpu->cpu_timer)) {
		...
		cpufreq_zerolag_timer(smp_processor_id());
	}

	up_read(&pcpu->enable_sem);
}

static int cpufreq_zerolag_speedchange_task(void *data)
{
	unsigned int cpu;
	cpumask_t tmp_mask;
	unsigned long flags;
	...
	while (1) {
		...
		for_each_cpu(cpu, &tmp_mask) {
			...
			for_each_cpu(j, pcpu->policy->cpus) {
				struct cpufreq_zerolag_cpuinfo *pjcpu =
					&per_cpu(cpuinfo, j);
			...
			if (max_freq != pcpu->policy->cur)
				__cpufreq_driver_target(pcpu->policy,
							max_freq,
							CPUFREQ_RELATION_H);
			trace_cpufreq_zerolag_setspeed(cpu,
						     pcpu->target_freq,
						     pcpu->policy->cur);
...
static void cpufreq_zerolag_boost(void)
{
	int i;
	int anyboost = 0;
	unsigned long flags;
...
static int cpufreq_zerolag_notifier(
...
static struct notifier_block cpufreq_notifier_block = {
	.notifier_call = cpufreq_zerolag_notifier,
};
			   ...
			   const char *buf, size_t count)
{
	...
	if (boost_val) {
		trace_cpufreq_zerolag_boost("on");
		cpufreq_zerolag_boost();
	} else {
		trace_cpufreq_zerolag_unboost("off");
	}
				...
				const char *buf, size_t count)
{
	...
	trace_cpufreq_zerolag_boost("pulse");
	cpufreq_zerolag_boost();
	return count;
}
...
static struct attribute *zerolag_attributes[] = {
...
static struct attribute_group zerolag_attr_group = {
	.attrs = zerolag_attributes,
	.name = "zerolag",
};

static int cpufreq_zerolag_idle_notifier(struct notifier_block *nb,
					     unsigned long val,
					     void *data)
{
	switch (val) {
	case IDLE_START:
		cpufreq_zerolag_idle_start();
		break;
	case IDLE_END:
		cpufreq_zerolag_idle_end();
		break;
	}
...
static struct notifier_block cpufreq_zerolag_idle_nb = {
	.notifier_call = cpufreq_zerolag_idle_notifier,
};
		...
		unsigned int event)
{
	...
	case CPUFREQ_GOV_START:
		...
		for_each_cpu(j, policy->cpus) {
			...
			cpufreq_zerolag_timer_start(j, 0);
		...
		idle_notifier_register(&cpufreq_zerolag_idle_nb);
	...
	case CPUFREQ_GOV_STOP:
		...
		cpufreq_unregister_notifier(
			&cpufreq_notifier_block, CPUFREQ_TRANSITION_NOTIFIER);
		idle_notifier_unregister(&cpufreq_zerolag_idle_nb);
		...
		for_each_cpu(j, policy->cpus) {
			...
			if (policy->min >= pcpu->target_freq) {
				...
				expire_time = pcpu->cpu_timer.expires - jiffies;
 				...
 				cpufreq_zerolag_timer_start(j, expire_time);
			}
...
static void cpufreq_zerolag_nop_timer(unsigned long data)
{
}

static int __init cpufreq_zerolag_init(void)
{
	unsigned int i;
	struct cpufreq_zerolag_cpuinfo *pcpu;
	...
	for_each_possible_cpu(i) {
		...
		pcpu->cpu_timer.function = cpufreq_zerolag_timer;
		pcpu->cpu_timer.data = i;
		init_timer(&pcpu->cpu_slack_timer);
		pcpu->cpu_slack_timer.function = cpufreq_zerolag_nop_timer;
	...
	mutex_init(&gov_lock);
	speedchange_task =
		kthread_create(cpufreq_zerolag_speedchange_task, NULL,
			       "cfzerolag");
	...
	return cpufreq_register_governor(&cpufreq_gov_zerolag);
}

#ifdef CONFIG_CPU_FREQ_DEFAULT_GOV_ZEROLAG
fs_initcall(cpufreq_zerolag_init);
#else
module_init(cpufreq_zerolag_init);
#endif

static void __exit cpufreq_zerolag_exit(void)
{
	cpufreq_unregister_governor(&cpufreq_gov_zerolag);
	kthread_stop(speedchange_task);
	put_task_struct(speedchange_task);
}

module_exit(cpufreq_zerolag_exit);

MODULE_AUTHOR("Mike Chan <mike@android.com>");
MODULE_DESCRIPTION("'cpufreq_zerolag' - A cpufreq governor for "
