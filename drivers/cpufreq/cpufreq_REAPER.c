...
static void cpufreq_reaper_timer_resched(
 ...
 * function.
 */
static void cpufreq_reaper_timer_start(int cpu)
{
	struct cpufreq_reaper_cpuinfo *pcpu = &per_cpu(cpuinfo, cpu);
...
static unsigned int choose_freq(
	struct cpufreq_reaper_cpuinfo *pcpu, unsigned int loadadjfreq)
{
...
static void cpufreq_reaper_timer(unsigned long data)
{
	...
	struct cpufreq_reaper_cpuinfo *picpu;
...
rearm:
	if (!timer_pending(&pcpu->cpu_timer))
		cpufreq_reaper_timer_resched(pcpu);
...
static void cpufreq_reaper_idle_end(void)
{
	...
	if (!timer_pending(&pcpu->cpu_timer)) {
		...
		cpufreq_reaper_timer(smp_processor_id());
	}

	up_read(&pcpu->enable_sem);
}

static int cpufreq_reaper_speedchange_task(void *data)
{
	unsigned int cpu;
	cpumask_t tmp_mask;
	unsigned long flags;
		...
		for_each_cpu(cpu, &tmp_mask) {
			...
			if (max_freq != pcpu->policy->cur)
				__cpufreq_driver_target(pcpu->policy,
							max_freq,
							CPUFREQ_RELATION_H);
			trace_cpufreq_reaper_setspeed(cpu,
						     pcpu->target_freq,
						     pcpu->policy->cur);
...
static int cpufreq_reaper_notifier(
...
static struct notifier_block cpufreq_notifier_block = {
	.notifier_call = cpufreq_reaper_notifier,
};
		...
		unsigned int event)
{
	...
	case CPUFREQ_GOV_STOP:
		...
		cpufreq_unregister_notifier(
			&cpufreq_notifier_block, CPUFREQ_TRANSITION_NOTIFIER);
		idle_notifier_unregister(&cpufreq_reaper_idle_nb);
...
static void cpufreq_reaper_nop_timer(unsigned long data)
{
}
...
static int __init cpufreq_reaper_init(void)
{
	...
	for_each_possible_cpu(i) {
		...
		pcpu->cpu_slack_timer.function = cpufreq_reaper_nop_timer;
	...
	mutex_init(&gov_lock);
	speedchange_task =
		kthread_create(cpufreq_reaper_speedchange_task, NULL,
			       "cfreaper");
